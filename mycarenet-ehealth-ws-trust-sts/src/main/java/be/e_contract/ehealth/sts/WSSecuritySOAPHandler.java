 ...
 * implementation uses WSS4J to create the WS-Security header.
 * 
...
public class WSSecuritySOAPHandler implements SOAPHandler<SOAPMessageContext> {
	...
	public void setCredential(PrivateKey privateKey, X509Certificate certificate) {
	...
	private void handleOutboundMessage(SOAPMessageContext context) throws WSSecurityException {
		...
		if (this.certificate.getPublicKey().getAlgorithm().equals("EC")) {
			sign.setSignatureAlgorithm(WSConstants.ECDSA_SHA256);
		} else {
